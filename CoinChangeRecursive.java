...
public class CoinChangeRecursive {
	...
	public static int coinChange(int[] coins, int amount) {
		...
		return findMinCoins(coins,amount,0,0);
	}
	
	private static int findMinCoins(int[] coins, int amount, int index, int minCoins) {
		
		
		if(amount == 0) return minCoins;
		if(amount<0 || index == coins.length) return -1;
		//case 1 choose the coin
		int case1 = findMinCoins(coins,amount-coins[index],index,minCoins+1);
		//case 2 not choose
		int case2 = findMinCoins(coins,amount,index+1,minCoins);
